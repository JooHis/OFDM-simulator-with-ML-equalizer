...
% pyver = pyversion('C:\Users\juham\AppData\Local\Programs\Python\Python39\python.exe');
...
N_Pilot = 16; % Number of pilot symbols (8 or 16)
...
berEst = zeros([size(snrVec), 5]); % Vector used to store BER estimates
...
%         xlabel('Subcarrier N')
%         ylabel('|H(f)|')
%         title('Channel frequency resonse equalization coefficients estimation, SNR 30, 16 pilots')
        ...
        % Get ML gray coding estimates
        rx_bins_eqML = [real(rx_bins_noise(Id)), imag(rx_bins_noise(Id)), real(hEstZfTemp(Id)), imag(hEstZfTemp(Id))*-1];
        est = pyrunfile("estimate.py","z", OFDM_symbol=rx_bins_eqML, pilots=N_Pilot);
        rx_bins_eqML = double(est);
        ...
        % Demodulate ML
        rx_bins_eqML = de2bi(rx_bins_eqML, k);
        rx_bitsML = rx_bins_eqML(:)';
        ...
        % ML equalization error
        [num_errML, berML] = biterr(tx_bits, rx_bitsML);
...
hold on
semilogy(snrVec, berEst(:, 5), '--x')
hold on
semilogy(snrVec, berEst(:, 4), 'Color', 'green')
hold on
grid
legend('No-EQ', 'ZF', 'MMSE', 'Deep NN', 'Full CSI knowledge')
xlabel('SNR (db)')
